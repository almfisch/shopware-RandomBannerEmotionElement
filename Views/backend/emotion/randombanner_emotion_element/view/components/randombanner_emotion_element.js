...
Ext.define('Shopware.apps.Emotion.view.components.RandomBanner', {
    extend: 'Shopware.apps.Emotion.view.components.Base',
    alias: 'widget.emotion-components-randombanner',
    ...
    onAddBannerToGrid: function(field, records) {
        ...
        Ext.each(records, function(record) {
            ...
            var model = Ext.create('Shopware.apps.Emotion.model.RandomBanner', {
                position: count,
    ...
    getGridData: function() {
        var me = this,
            elementStore = me.getSettings('record').get('data'), randomBanner;

        Ext.each(elementStore, function(element) {
            if(element.key === 'random_banner') {
                randomBanner = element;
                return false;
            }
        });

        if(randomBanner && randomBanner.value) {
            Ext.each(randomBanner.value, function(item) {
                me.bannerStore.add(Ext.create('Shopware.apps.Emotion.model.RandomBanner', item));
            });
        }
    },
